...
nat = xr.open_dataset(path + 'NAT.nc') #'NAT1_AOD550.nc'
...
def create_input_images(df, input_label):
    ...
    if input_label == 'spex_3_day':
        for i in df.index:
            input_image.append(df[input_label][i].reshape(96, 192, 3))
    else:
        for i in df.index:
...
def parse_combined_data(spex_day, ctl_lat_grad, ctl_lon_grad, ctl_day, nat_day):
    ...
    data = {
        ...
        'nat_day': _bytes_feature(serialize_array(nat_day)),
    }
...
def write_data(spex_day, ctl_lat_grad, ctl_lon_grad, ctl_day, nat_day,
...
for n in range(n_days):
    ctl_day_avg[n] = ctl.TAU_2D_550nm.data[n]
    ...
    nat_day_avg[n] = nat.TAU_2D_550nm.data[n]
    spex_day_avg[n] = spex_one.Count.data[n]
    spex_day_avg[n] = np.where(np.isnan(spex_day_avg[n]), spex_day_avg[n],
                               nat_day_avg[n] * spex_day_avg[n])
    spex_day_avg[n] = np.nan_to_num(spex_day_avg[n], nan=-1).reshape(96, 192, 1)
...
dataframe = dataframe.assign(CTL_lat_grad = ctl_day_lat_grad.values(),
                             ...
                             spex_3_day = spex_3_day.values(),
                             nat_day_avg = nat_day_avg.values())
...
ctl_test = create_input_images(test_data, "CTL daily avg")
spex_day_avg_test = create_input_images(test_data, 'spex_3_day')
spex_day_avg_test = spex_day_avg_test.astype("float32")

nat_test = create_output(test_data, "nat_day_avg")
...
ctl_train = create_input_images(train_data, "CTL daily avg")
spex_day_avg_train = create_input_images(train_data, 'spex_3_day')
spex_day_avg_train = spex_day_avg_train.astype("float32")

nat_train = create_output(train_data, "nat_day_avg")

#%% Write to tfrecord
write_data(spex_day_avg_test, ctl_lat_grad_test, ctl_lon_grad_test,
           ctl_test, nat_test, max_files=100,
           filename='test_data_3days', out_dir=path+"test_data_3days/")

write_data(spex_day_avg_train, ctl_lat_grad_train, ctl_lon_grad_train,
           ctl_train, nat_train, max_files=100,
           filename='train_data_3days', out_dir=path+"train_data_3days/")
